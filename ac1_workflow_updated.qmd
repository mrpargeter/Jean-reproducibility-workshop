...
# ---- Source URL (latest) ----
ss_url <- "https://docs.google.com/spreadsheets/d/1sRHgcU5-vPCkm51F-gCsD4-cEE6MGqXrq6uyrp3ZjAI/edit?usp=sharing"
```
...
ac1_heat_df <- ac1_summary %>%
  mutate(category = factor(category, levels = c("Flakes","Cores")),
         Attribute = fct_reorder(Attribute, ac1, .fun = mean, .desc = TRUE))

p_heat <- ggplot(ac1_heat_df, aes(x = category, y = Attribute, fill = ac1)) +
  ...
  labs(title = "Agreement (AC1) by attribute and type", x = NULL, y = NULL, fill = "AC1") +
  theme_minimal(base_size = 12) +
